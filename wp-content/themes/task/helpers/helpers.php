...
class Helpers
{
  ...
  public static function get_post_thumbnail($post_ID, $size = 'medium', $attachment_attributes = array())
  {
    ...
    return Helpers::get_image($post_thumbnail_id, $size, $attachment_attributes) ?: '<img class="no-image" src="' . DH_IMAGE_DIR . '/no-image.png" alt="No Image">';
  ...
  public static function display_attachment($attachment_id, $attachment_attributes = array())
  {
    ...
    if ($attachment_mime_type !== 'image/svg+xml') {
      $img_attr = '';
      if (is_array($attachment_attributes) && !empty($attachment_attributes)) {
