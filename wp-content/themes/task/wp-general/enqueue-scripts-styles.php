...
add_action('wp_enqueue_scripts', 'webofai_theme_scripts');
function webofai_theme_scripts()
{
...
//  wp_enqueue_script('algolia-search-loader', get_stylesheet_directory_uri() . '/algolia-search-loader.js', array('jquery', 'algolia-autocomplete'), '1.0.0', true);
}
...
function add_admin_style_to_post_page_only()
{
  ...
  wp_enqueue_script('admin', get_template_directory_uri() . '/assets/admin.js', array(), time());

  wp_enqueue_style('admin', get_template_directory_uri() . '/assets/admin.css', array(), time());
