...
//region Rewebofai_theme Paragraph Tags - ACF shortcode [check]
...
//endregion Rewebofai_theme Paragraph Tags - ACF shortcode [check]
...
{
  add_menu_page('Global Settings & Styles', 'Global Styles', 'manage_options', site_url() . '/global-settings.php', '', 'dashicons-admin-generic', 63.3);
}

// endregion add global settings and styles to admin menu

// add author column to tool CPT
function tool_add_author_column($columns)
{
  $columns['author'] = __('Author', 'text-domain');
  return $columns;
}

add_filter('manage_tool_posts_columns', 'tool_add_author_column');

// display author name in author column
function tool_custom_columns($column, $post_id)
{
  if ($column === 'author') {
...
add_action('manage_tool_posts_custom_column', 'tool_custom_columns', 10, 2);

function update_post_views()
{
  if (is_single()) { // Only update view count on single post pages
    $post_id = get_the_ID();
    $view_count = get_post_meta($post_id, 'view_count', true);
    if (!$view_count) {
      $view_count = 1;
      add_post_meta($post_id, 'view_count', $view_count);
    } else {
      $view_count++;
